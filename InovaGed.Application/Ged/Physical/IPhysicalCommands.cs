...
public interface IPhysicalCommands
{
    ...
    Task<Result> SetLocationActiveAsync(Guid tenantId, Guid id, bool active, Guid? userId, string? reason, CancellationToken ct);
    ...
    Task<Result> SetBoxStateAsync(Guid tenantId, Guid id, string state, bool isFull, Guid? userId, string? reason, CancellationToken ct);
