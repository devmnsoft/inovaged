(function(){
  ...
  function validateFiles(){if(!document.getElementById('bulkFolderId')?.value){showBulkUploadMessage('Selecione uma pasta antes de enviar.','warning');window.showAppToast?.('Selecione uma pasta antes de enviar.','warning','Validação');return false;} if(state.files.length===0){showBulkUploadMessage('Selecione pelo menos um arquivo.','warning');window.showAppToast?.('Selecione pelo menos um arquivo.','warning','Validação');return false;} return true;}
  async function uploadFiles(){ if(state.uploading||!validateFiles()) return; state.uploading=true; setUploadLoading(true); clearBulkUploadMessage(); const folderId=document.getElementById('bulkFolderId').value; const batchId=crypto.randomUUID(); let ok=0,err=0; for(let i=0;i<state.files.length;i++){const r=await uploadSingleFile(state.files[i],folderId,batchId,i); if(r)ok++; else err++;} const msg=err>0?'Alguns arquivos falharam.':(ok>1?'Documentos enviados com sucesso.':'Arquivo enviado com sucesso.'); showBulkUploadMessage(msg, err?'warning':'success'); window.showAppToast?.(msg, err?'warning':'success', 'Upload em lote'); state.uploading=false; setUploadLoading(false); if(ok>0&&err===0){ setTimeout(()=>window.location.reload(),600);} }
  function uploadSingleFile(item,folderId,batchId,index){ return new Promise((resolve)=>{ item.status='Enviando'; renderFileList(); const xhr=new XMLHttpRequest(); xhr.open('POST','/Ged/Documents/BulkUploadSingle'); xhr.upload.onprogress=(e)=>{ if(e.lengthComputable) updateFileProgress(index,Math.round((e.loaded/e.total)*100)); }; xhr.onload=()=>{ try{const j=JSON.parse(xhr.responseText||'{}'); if(xhr.status>=200&&xhr.status<300&&j.success){ item.status='Enviado'; item.progress=100; resolve(true);} else { item.error=j.message||'Falha no envio'; item.status='Erro'; resolve(false);} }catch{ item.status='Erro'; item.error='Resposta inválida'; resolve(false);} renderFileList();}; xhr.onerror=()=>{item.status='Erro';item.error='Erro de rede';renderFileList();resolve(false);}; const fd=new FormData(); fd.append('file',item.file); fd.append('folderId',folderId); fd.append('batchId',batchId); const token=document.querySelector('input[name="__RequestVerificationToken"]')?.value; if(token) xhr.setRequestHeader('RequestVerificationToken',token); xhr.send(fd); }); }
  ...
  function clearBulkUploadMessage(){const el=document.getElementById('bulkUploadMessage'); if(!el) return; el.classList.add('d-none'); el.textContent='';}
  function setUploadLoading(isLoading){ const btn=document.getElementById('btnBulkSend'); if(!btn) return; btn.disabled=!!isLoading; btn.innerHTML=isLoading?'<span class="spinner-border spinner-border-sm me-1"></span>Enviando...':'Enviar arquivos'; }
