(() => {
  ...
  form.addEventListener('submit', async event => {
    ...
    try {
      const body = new FormData(form); body.set('question', question);
  ...
  function render(response) {
    ...
    feed.insertAdjacentHTML('beforeend', `<article class="assistant-message bot"><span>Assistente documental</span><p>${escapeHtml(response.answer)}</p><div class="assistant-criteria"><strong>Critérios utilizados</strong><p>${escapeHtml(response.criteria)}</p></div>${sources ? `<div class="assistant-sources"><h2>Fontes encontradas (${response.total})</h2>${sources}</div>` : ''}</article>`);
