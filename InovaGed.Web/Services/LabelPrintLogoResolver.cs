...
public sealed class LabelPrintLogoResolver(IDbConnectionFactory factory, IBrandAssetImageService images) : ILabelPrintLogoResolver
{
 public async Task<ResolvedPrintLogo> ResolveAsync(Guid tenantId,string templateCode,string selection,Guid? selectedAssetId,decimal? widthMm,decimal? heightMm,bool preserveAspectRatio,string? fitMode,string? position,decimal offsetXmm,decimal offsetYmm,CancellationToken ct){
  selection=(selection??"TEMPLATE_DEFAULT").Trim().ToUpperInvariant();
  // A escolha explícita vence inclusive valores antigos de LogoSelection.
  if(selectedAssetId is null && selection=="NONE")return Empty(); await using var db=await factory.OpenAsync(ct);
  if(!await db.ExecuteScalarAsync<bool>(new CommandDefinition("select to_regclass('ged.brand_asset') is not null",cancellationToken:ct)))return Empty(); Guid? id=selectedAssetId; // A escolha explícita sempre vence os fallbacks, independentemente da origem visual enviada.
  if(id is null && selection is ("TEMPLATE_DEFAULT" or "PROFILE") && await db.ExecuteScalarAsync<bool>(new CommandDefinition("select to_regclass('ged.print_template_brand_binding') is not null",cancellationToken:ct))) id=await db.QueryFirstOrDefaultAsync<Guid?>(new CommandDefinition("select b.logo_asset_id from ged.print_template_brand_binding b join ged.brand_asset a on a.id=b.logo_asset_id and (a.tenant_id=@tenantId or a.tenant_id is null) and a.status='ACTIVE' and a.reg_status='A' where (b.tenant_id=@tenantId or b.tenant_id is null) and b.template_code=@templateCode and b.enabled=true and b.reg_status='A' order by b.tenant_id nulls last limit 1",new{tenantId,templateCode},cancellationToken:ct));
  ...
  var r=await db.QuerySingleOrDefaultAsync<Row>(new CommandDefinition("select id,brand_name BrandName,asset_name AssetName,default_width_mm DefaultWidthMm,default_height_mm DefaultHeightMm,preserve_aspect_ratio PreserveAspectRatio,fit_mode FitMode,default_position DefaultPosition,alt_text AltText from ged.brand_asset where id=@id and (tenant_id=@tenantId or tenant_id is null) and status='ACTIVE' and reg_status='A'",new{id,tenantId},cancellationToken:ct)); if(r is null)return Empty();
