...
public sealed class LabelPrintLogoResolver(IDbConnectionFactory factory, IPrintLogoImageSourceBuilder imageSources, IHttpContextAccessor httpContextAccessor, IUrlHelperFactory urlHelperFactory) : ILabelPrintLogoResolver
{
 public async Task<ResolvedPrintLogo> ResolveAsync(Guid tenantId,string templateCode,string selection,Guid? selectedAssetId,decimal? widthMm,decimal? heightMm,bool preserveAspectRatio,string? fitMode,string? position,decimal offsetXmm,decimal offsetYmm,CancellationToken ct){
  ...
  var actionContext = new ActionContext(httpContextAccessor.HttpContext!, httpContextAccessor.HttpContext!.GetRouteData(), new Microsoft.AspNetCore.Mvc.Abstractions.ActionDescriptor());
  var url = urlHelperFactory.GetUrlHelper(actionContext);
  var webUrl = await imageSources.BuildWebUrlAsync(tenantId,r.Id,url,ct);
  var printSource = await imageSources.BuildPrintImageSourceAsync(tenantId,r.Id,url,ct);
  var hasLogo = !string.IsNullOrWhiteSpace(webUrl) || !string.IsNullOrWhiteSpace(printSource);
  return new(r.Id,r.BrandName,webUrl,printSource,string.IsNullOrWhiteSpace(r.AltText)?r.AssetName:r.AltText,widthMm is >0?widthMm.Value:r.DefaultWidthMm,heightMm is >0?heightMm:r.DefaultHeightMm,preserveAspectRatio,fit,pos,offsetXmm,offsetYmm,hasLogo);
 }
 private static ResolvedPrintLogo Empty()=>new(null,null,null,null,"Sem logo",38,null,true,"CONTAIN","TOP_LEFT",0,0,false);
