...
public interface ILabelPrintLogoResolver { Task<ResolvedPrintLogo> ResolveAsync(Guid tenantId,string templateCode,string selection,Guid? selectedAssetId,decimal? widthMm,decimal? heightMm,bool preserveAspectRatio,string? fitMode,string? position,CancellationToken ct); }
public sealed class LabelPrintLogoResolver(IDbConnectionFactory factory) : ILabelPrintLogoResolver
{
 public async Task<ResolvedPrintLogo> ResolveAsync(Guid tenantId,string templateCode,string selection,Guid? selectedAssetId,decimal? widthMm,decimal? heightMm,bool preserveAspectRatio,string? fitMode,string? position,CancellationToken ct){
  ...
  return new(r.Id,r.BrandName,$"/Administration/BrandAssets/{r.Id}/File",string.IsNullOrWhiteSpace(r.AltText)?r.AssetName:r.AltText,widthMm is >0?widthMm.Value:r.DefaultWidthMm,heightMm is >0?heightMm:r.DefaultHeightMm,preserveAspectRatio,fit,pos,0,0,true);
 }
 private static ResolvedPrintLogo Empty()=>new(null,null,null,"Sem logo",38,null,true,"CONTAIN","TOP_LEFT",0,0,false);
