...
public sealed class ProtocoloCadastrosController : GedControllerBase
{
    ...
    [HttpGet] public async Task<IActionResult> EditarUsuarioSetor(Guid id) { using var db = await OpenAsync(); var vm = await db.QuerySingleOrDefaultAsync<ProtocoloUsuarioSetorFormVM>("select id Id,usuario_id UsuarioId,usuario_nome UsuarioNome,setor_id SetorId,ativo Ativo from ged.protocolo_usuario_setor where tenant_id=@TenantId and id=@Id and reg_status='A'", new { TenantId, Id = id }); if (vm == null) return NotFound(); vm.Setores = await Setores(db, vm.SetorId); return View("UsuarioSetorForm", vm); }
