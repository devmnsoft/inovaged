...
public sealed class UploadChunkController : Controller
{
    ...
    public sealed class StartRequest : StartUploadChunkRequestDto { }
    ...
    public async Task<IActionResult> Start([FromBody] StartRequest request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var folder = await ResolveUploadFolderAsync(request.FolderId, request.RequestedFolderId, isAdmin, correlationId, ct);
            ...
            request.FolderId = folder.ResolvedFolderId;
            request.RequestedFolderId = folder.RequestedFolderId;
            request.CorrelationId = correlationId;
            request.UserName = User.Identity?.Name;
            request.IsAdmin = isAdmin;
            var result = await _chunks.StartAsync(_currentUser.TenantId, _currentUser.UserId, request, ct);
        ...
        {
            _logger.LogError(ex, "Erro ao iniciar upload chunked. Tenant={TenantId} User={UserId} CorrelationId={CorrelationId}", _currentUser.TenantId, _currentUser.UserId, correlationId);
