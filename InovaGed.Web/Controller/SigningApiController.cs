    ...
    IOptions<DigitalSignatureOptions> options) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateSigningSessionRequest request, CancellationToken ct)
    {
        ...
        return Ok(new CreateSigningSessionResponse(result.SessionId.Value, SigningProcessStatus.REQUESTED.ToString(), $"/api/signing/sessions/{result.SessionId.Value}/content", contentToken, completionToken, sha256, metadata.SizeBytes, metadata.FileName, metadata.DocumentCode, metadata.VersionLabel, expires, HttpContext.TraceIdentifier));
    }
    ...
    {
        Response.Headers.CacheControl = "no-store";
        var tenantId = ReadGuidClaim("tenant_id"); if (tenantId is null || string.IsNullOrWhiteSpace(token)) return Unauthorized();
        var tokenHash = Hash(token); var s = await sessions.GetForContentAsync(tenantId.Value, id, tokenHash, ct); if (s is null) return Unauthorized();
        if (!await sessions.ConsumeContentTokenAsync(tenantId.Value, id, tokenHash, ct)) return Conflict(new { error = "content_token_already_consumed_or_invalid_state" });
        var meta = await content.GetMetadataAsync(tenantId.Value, s.DocumentId, s.DocumentVersionId, ct);
        var stream = await content.OpenReadAsync(tenantId.Value, s.DocumentId, s.DocumentVersionId, ct);
    ...
    public async Task<IActionResult> Complete(Guid id, [FromBody] CompleteSigningSessionRequest request, CancellationToken ct)
    {
        ...
        var command = new CompleteSignatureCommand(id, cms, cert, chain, null, new Dictionary<string, string> { ["completion_token_hash"] = Hash(request.CompletionToken), ["idempotency_key"] = request.IdempotencyKey, ["agent_operation_id"] = request.AgentOperationId, ["agent_version"] = request.AgentVersion, ["tenant_id"] = ReadGuidClaim("tenant_id")?.ToString() ?? "", ["user_id"] = (ReadGuidClaim("sub") ?? ReadGuidClaim(System.Security.Claims.ClaimTypes.NameIdentifier))?.ToString() ?? "" });
