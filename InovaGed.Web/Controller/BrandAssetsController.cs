...
public sealed class BrandAssetsController(IDbConnectionFactory dbFactory, IWebHostEnvironment environment, IConfiguration configuration, IBrandAssetImageService images) : GedControllerBase(dbFactory)
{
    ...
    public async Task<IActionResult> Index(CancellationToken ct)
    {
        ...
        return View("~/Views/Administration/BrandAssets/Index.cshtml", rows.AsList());
    }
    ...
    public async Task<IActionResult> Create(BrandAssetUploadInput input, CancellationToken ct)
    {
        ...
        if (!ModelState.IsValid) return View("~/Views/Administration/BrandAssets/Create.cshtml", input);
        ...
        if (!signatureOk) { ModelState.AddModelError(nameof(input.File), "O conteúdo do arquivo não corresponde ao formato informado."); return View("~/Views/Administration/BrandAssets/Create.cshtml", input); }
        ...
        await db.ExecuteAsync(new CommandDefinition("insert into ged.brand_asset(id,tenant_id,brand_name,asset_name,original_file_name,stored_file_name,content_type,file_extension,file_size_bytes,file_hash_sha256,storage_relative_path,public_route,is_default,default_width_mm,default_height_mm,preserve_aspect_ratio,fit_mode,notes,created_by,created_by_name) values(@id,@tenant,@brand,@asset,@original,@stored,@content,@extension,@size,@hash,@relative,@route,@isDefault,@width,@height,@preserve,@fit,@notes,@userId,@userName)", new { id,tenant=TenantId,brand=input.BrandName.Trim(),asset=input.AssetName.Trim(),original=Path.GetFileName(file.FileName),stored=storedName,content=expected.ContentType,extension=Path.GetExtension(storedName),size=file.Length,hash,relative,route=$"/Administration/BrandAssets/{id}/File",isDefault=input.IsDefault,width=input.DefaultWidthMm,height=input.DefaultHeightMm,preserve=input.PreserveAspectRatio,fit=input.FitMode,notes=input.Notes,userId=UserId,userName=UserNameSafe },tx,cancellationToken:ct));
    ...
    public async Task<IActionResult> Edit(Guid id, CancellationToken ct)
    {
        ...
        return View("~/Views/Administration/BrandAssets/Edit.cshtml", new BrandAssetEditInput { Id=asset.Id, BrandName=asset.BrandName, AssetName=asset.AssetName,
    ...
    public async Task<IActionResult> Edit(Guid id, BrandAssetEditInput input, CancellationToken ct)
    {
        ...
        if (!ModelState.IsValid) return View("~/Views/Administration/BrandAssets/Edit.cshtml", input);
