    ...
    ILogger<LabelCanvasDesignRepository> logger) : ILabelCanvasDesignService
{
    ...
    public async Task<LabelCanvasDesignDto> CreateDraftAsync(Guid tenantId,Guid userId,LabelCanvasSaveRequest request,string? ipAddress,string? userAgent,CancellationToken cancellationToken=default)
    {
        ...
        try
        {
            ...
            const string sql="""
insert into ged.label_template_design(id,tenant_id,template_key,template_code,template_name,description,template_kind,print_mode,subject_type,paper_kind,paper_size,width_mm,height_mm,orientation,status,design_json,current_version,is_system_template,created_by,created_at,reg_status)
values(@id,@tenantId,@TemplateKey,@TemplateKey,@TemplateName,@Description,@TemplateKind,'CUSTOM',@SubjectType,@PaperKind,@PaperKind,@WidthMm,@HeightMm,@Orientation,'DRAFT',cast(@DesignJson as jsonb),1,false,@userId,now(),'ACTIVE');
""";
...
""";
    ...
    private static LabelCanvasDesignDto VersionDesign(LabelCanvasDesignDto source,string json,int version)=>new(){Id=source.Id,TenantId=source.TenantId,TemplateKey=source.TemplateKey,TemplateName=source.TemplateName,Description=source.Description,TemplateKind=source.TemplateKind,SubjectType=source.SubjectType,PaperKind=source.PaperKind,WidthMm=source.WidthMm,HeightMm=source.HeightMm,Orientation=source.Orientation,Status="PUBLISHED",DesignJson=json,CurrentVersion=version,IsSystemTemplate=true,CreatedBy=source.CreatedBy,CreatedAt=source.CreatedAt,PublishedBy=source.PublishedBy,PublishedAt=source.PublishedAt};
    private static LabelCanvasSaveRequest SaveRequest(LabelCanvasDesignDto design,string? summary)=>new(){TemplateKey=design.TemplateKey,TemplateName=design.TemplateName,Description=design.Description,TemplateKind=design.TemplateKind,SubjectType=design.SubjectType,PaperKind=design.PaperKind,WidthMm=design.WidthMm,HeightMm=design.HeightMm,Orientation=design.Orientation,DesignJson=design.DesignJson,ChangeSummary=summary};
    private static DynamicParameters Parameters(LabelCanvasSaveRequest request,Guid tenantId,Guid userId,Guid id){var p=new DynamicParameters();p.Add("id",id,DbType.Guid);p.Add("tenantId",tenantId,DbType.Guid);p.Add("userId",userId,DbType.Guid);p.Add("TemplateKey",request.TemplateKey,DbType.String);p.Add("TemplateName",request.TemplateName,DbType.String);p.Add("Description",request.Description,DbType.String);p.Add("TemplateKind",request.TemplateKind,DbType.String);p.Add("SubjectType",request.SubjectType,DbType.String);p.Add("PaperKind",request.PaperKind,DbType.String);p.Add("WidthMm",request.WidthMm,DbType.Decimal);p.Add("HeightMm",request.HeightMm,DbType.Decimal);p.Add("Orientation",request.Orientation,DbType.String);p.Add("DesignJson",request.DesignJson,DbType.String);return p;}
