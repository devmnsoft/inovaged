...
public sealed class GovernanceService(IDbConnectionFactory db) : IGovernanceDashboardService, IGovernanceAuditService, IGovernanceAlertService, IGovernanceEvidenceService, IGovernanceReportService
{
    ...
    Task<IReadOnlyList<GovernanceAuditItem>> IGovernanceAuditService.ListAsync(GovernanceAuditFilter filter, CancellationToken ct)
        => ListAuditAsync(filter, ct);

    private async Task<IReadOnlyList<GovernanceAuditItem>> ListAuditAsync(GovernanceAuditFilter f, CancellationToken ct)
    {
        await using var c = await db.OpenAsync(ct);
...
""", f, cancellationToken: ct));
    ...
    Task<IReadOnlyList<GovernanceAlertItem>> IGovernanceAlertService.ListAsync(GovernanceAlertFilter filter, CancellationToken ct)
        => ListAlertsAsync(filter, ct);

    private async Task<IReadOnlyList<GovernanceAlertItem>> ListAlertsAsync(GovernanceAlertFilter f, CancellationToken ct)
    {
    ...
    public async Task<GovernanceReportResult> GenerateAsync(GovernanceReportQuery q,CancellationToken ct){if(!Reports.TryGetValue(q.ReportType,out var report))throw new ArgumentException("Tipo de relatório inválido.");var filter=new GovernanceAlertFilter(q.TenantId,Type:report.AlertType,From:q.From,To:q.To);var alerts=await ListAlertsAsync(filter,ct);return new(q.ReportType,report.Title,alerts.Select(x=>new GovernanceReportRow(x.SourceId?.ToString()[..8]??"—",Mask(x.Title),x.Status,x.CreatedAt)).ToArray(),true);}
