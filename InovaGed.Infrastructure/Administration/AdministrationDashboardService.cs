...
public sealed class AdministrationDashboardService : IAdministrationDashboardService
{
    ...
    public async Task<AdministrationOverview> GetOverviewAsync(Guid? tenantId, CancellationToken ct = default)
    {
        ...
        {
            await Count(c,"active_users","Usuários ativos","app_user","is_active=true and deleted_at_utc is null",tenantId,ct,"bi-people"),
            await Count(c,"blocked_users","Usuários bloqueados","app_user","coalesce(is_locked,false)=true and deleted_at_utc is null",tenantId,ct,"bi-person-lock"),
            await Count(c,"active_tenants","Tenants ativos","tenant","is_active=true",null,ct,"bi-building"),
    ...
    public async Task<IReadOnlyList<PermissionCatalogItem>> GetPermissionCatalogAsync(string? search, CancellationToken ct = default) { await using var c=await _db.OpenAsync(ct); var table=await Table(c,"permission",ct)?"permission":null; if(table is null) return Array.Empty<PermissionCatalogItem>(); return (await c.QueryAsync<PermissionCatalogItem>(new CommandDefinition("select code Code, coalesce(description,code) Description, coalesce(module,'Geral') Module, '' Roles, 0 UsersAffected, coalesce(reg_status,'ATIVO') Status, 'Banco' Origin, null::timestamptz LastChangedAt from ged.permission where (@search is null or code ilike '%'||@search||'%' or description ilike '%'||@search||'%') order by module, code limit 200", new{search}, cancellationToken:ct))).ToList(); }
    public async Task<IdentityMigrationSummary> GetIdentityMigrationSummaryAsync(Guid? tenantId, CancellationToken ct = default) { await using var c=await _db.OpenAsync(ct); var total=await SafeInt(c,"app_user","deleted_at_utc is null",tenantId,ct); var migrated=await SafeInt(c,"user_identity_document","document_type='CPF'",tenantId,ct); return new(total,0,migrated,0,Math.Max(0,total-migrated),0,0,Math.Max(0,total-migrated)); }
    public Task<IReadOnlyList<AdministrationListItem>> GetUsersAsync(Guid? t,CancellationToken ct=default)=>List("app_user","coalesce(name,user_name,email,id::text)","coalesce(reg_status,'ATIVO')","coalesce(email,'Sem e-mail')",t,ct);
    ...
    public Task<IReadOnlyList<AdministrationListItem>> GetTenantsAsync(Guid? t,bool g,CancellationToken ct=default)=>List("tenant","coalesce(name,code,id::text)","coalesce(reg_status,'ATIVO')","coalesce(code,id::text)",g?null:t,ct);
