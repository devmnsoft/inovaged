...
public sealed class DocumentBulkUploadService : IDocumentBulkUploadService
{
    ...
    public async Task<Result<DocumentBulkUploadResultDto>> UploadSingleAsync(Guid tenantId, Guid userId, string? userName, IFormFile file, Guid? folderId, DocumentBulkUploadMetadata metadata, bool isAdmin, CancellationToken ct)
    {
        try
        {
            ...
            sw.Stop();
            _logger.LogInformation("Upload finalizado. Tenant={TenantId} User={UserId} Folder={FolderId} File={FileName} FileSize={FileSize} ContentType={ContentType} DuplicateStrategy={DuplicateStrategy} RunOcr={RunOcr} GeneratePreview={GeneratePreview} DocumentId={DocumentId} ElapsedMs={ElapsedMs}",
                tenantId, userId, folderId, safeName, file.Length, file.ContentType, metadata.DuplicateStrategy, metadata.RunOcr, metadata.GeneratePreview, result.Value, sw.ElapsedMilliseconds);
        ...
        } catch (Exception ex) { _logger.LogError(ex, "Erro em UploadSingleAsync. Tenant={TenantId} User={UserId} Folder={FolderId} Batch={BatchId} File={FileName}", tenantId, userId, folderId, metadata.BatchId, file?.FileName); return Result<DocumentBulkUploadResultDto>.Fail("ERR", "Erro interno ao enviar arquivo."); }
    }
