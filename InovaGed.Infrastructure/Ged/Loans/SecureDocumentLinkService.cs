...
public sealed class SecureDocumentLinkService : ISecureDocumentLinkService
{
    ...
    public async Task<SecureDocumentLinkResult> CreateAsync(Guid tenantId, Guid userId, CreateSecureDocumentLinkRequest request, CancellationToken ct)
    {
        ...
        if (!request.IsPermanent && request.ExpiresAtUtc is null) throw new InvalidOperationException("Expiração obrigatória para link temporário.");
        ...
        {
            var versionOk = await conn.ExecuteScalarAsync<bool>(new CommandDefinition("select exists(select 1 from ged.document_version where tenant_id=@tenantId and id=@versionId and document_id=@documentId)", new { tenantId, request.DocumentId, versionId = request.VersionId }, cancellationToken: ct));
            if (!versionOk) throw new InvalidOperationException("Versão não pertence ao documento informado.");
        }
        else
        {
            request.VersionId = await conn.ExecuteScalarAsync<Guid?>(new CommandDefinition("select id from ged.document_version where tenant_id=@tenantId and document_id=@documentId order by coalesce(version_no,0) desc, created_at desc limit 1", new { tenantId, request.DocumentId }, cancellationToken: ct));
        }
...
""", new { tenantId, loanRequestId = request.LoanRequestId, request.DocumentId, request.VersionId, hash, publicUrl, request.Title, request.Description, request.RecipientName, request.RecipientContact, request.IsPermanent, expiresAt = request.IsPermanent ? null : request.ExpiresAtUtc, maxAccess, request.AllowPreview, request.AllowDownload, request.AllowSmartSearch, userId }, cancellationToken: ct));
...
""", new { tenantId, loanId = request.LoanRequestId, linkId, userId, msg = request.MessageToRequester }, cancellationToken: ct));
        ...
        return new SecureDocumentLinkResult { LinkId = linkId, PublicUrl = publicUrl, ExpiresAtUtc = request.IsPermanent ? null : request.ExpiresAtUtc, IsPermanent = request.IsPermanent, MaxAccessCount = maxAccess };
    }
    ...
    public async Task<SecureDocumentLinkValidationResult> ValidateTokenAsync(string token, string? ip, string? userAgent, bool countAccess, CancellationToken ct)
    {
        ...
        string? denied = row.RevokedAt is not null ? "REVOKED" : (!row.IsPermanent && row.ExpiresAtUtc is null ? "INVALID_EXPIRATION" : (!row.IsPermanent && row.ExpiresAtUtc <= DateTimeOffset.UtcNow ? "EXPIRED" : (row.MaxAccessCount.HasValue && row.AccessCount >= row.MaxAccessCount ? "ACCESS_LIMIT" : null)));
