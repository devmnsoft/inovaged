...
public sealed class DispositionReportsQueries : IDispositionReportsQueries
{
    ...
    // ✅ fallback para status nulo (se a view ainda não calcula status)
    private const string DefaultStatus = "REVIEW_REQUIRED";
    ...
    public async Task<DispositionKpiVM> GetKpisAsync(Guid tenantId, CancellationToken ct)
    {
        try
        {
            // ✅ KPI na VIEW (mesma origem do grid)
            // ✅ disposition_status NULL -> REVIEW_REQUIRED (não zera)
            const string sql = """
            select
              coalesce(sum(case when coalesce(q.disposition_status, @defaultStatus) = 'ELIMINATION_READY' then 1 else 0 end),0)::bigint as "EliminationReady",
              coalesce(sum(case when coalesce(q.disposition_status, @defaultStatus) = 'TRANSFER_READY' then 1 else 0 end),0)::bigint as "TransferReady",
              coalesce(sum(case when coalesce(q.disposition_status, @defaultStatus) = 'REVIEW_REQUIRED' then 1 else 0 end),0)::bigint as "ReviewRequired",
              coalesce(sum(case when q.disposition_at >= (now() at time zone 'utc') - interval '30 days' then 1 else 0 end),0)::bigint as "DisposedLast30D"
            from ged.vw_disposition_queue q
            ...
            var vm = await conn.QuerySingleAsync<DispositionKpiVM>(
                new CommandDefinition(sql, new { tenantId, defaultStatus = DefaultStatus }, cancellationToken: ct));
...
select
  ...
  coalesce(disposition_status, @defaultStatus) as ""DispositionStatus"",
...
";
        ...
        p.Add("defaultStatus", DefaultStatus);

        if (!string.IsNullOrWhiteSpace(f.Status))
        {
            // ✅ filtra usando o mesmo fallback
            sql += " and coalesce(disposition_status, @defaultStatus) = @status ";
        ...
        Guid tenantId, DispositionFilter f, int page, int pageSize, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(f.Status))
        {
            // ✅ filtra usando fallback
            where += " and coalesce(disposition_status, @defaultStatus)=@status ";
            p.Add("status", f.Status);
        }
