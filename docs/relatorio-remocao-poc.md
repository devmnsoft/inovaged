...
Foram pesquisados termos relacionados a PoC, prova de conceito, demo, demonstração, sample, mock, fake, fictício, simulado, lorem ipsum, placeholder e exemplo.

## 3. Arquivos alterados

| Arquivo | Termo removido | Substituição aplicada | Motivo |
|---|---|---|---|
| `InovaGed.Web/Views/Account/Login.cshtml` | demo em comentário operacional | valores reais de tenant | Evitar orientação com tenant artificial. |
| `InovaGed.Web/Views/Manual/Index.cshtml` | demonstração institucional | padronização institucional | Remover linguagem de apresentação artificial. |
| `InovaGed.Web/Common/ICertificateValidationService.cs` | SQL de mock | script operacional de segurança | Remover referência técnica provisória. |
...
| `database/diagnostics/diagnostico_referencias_poc.sql` | novo | consultas diagnósticas | Localizar registros residuais no banco. |
| `database/migrations/2026_06_remove_poc_references.sql` | novo | saneamento controlado | Limpar dados operacionais após diagnóstico e backup. |
...
Não foram encontrados itens de menu com rótulo PoC. Textos operacionais revisados foram renomeados para termos institucionais.

## 5. Scripts SQL alterados

- `gedscript.sql`: removido marcador artificial de certificado e assinatura interna.
- `database/diagnostics/diagnostico_referencias_poc.sql`: criado para gerar consultas de localização.
- `database/migrations/2026_06_remove_poc_references.sql`: criado com transação e updates controlados.
