...
public sealed class LabelStudioProRc28Tests
{
    ...
    [Fact] public void generated_template_key_is_opaque_valid_and_unique(){var first=LabelTemplateKeyPolicy.Generate();var second=LabelTemplateKeyPolicy.Generate();Assert.StartsWith("LBL_",first);Assert.True(LabelTemplateKeyPolicy.IsValid(first));Assert.NotEqual(first,second);Assert.DoesNotContain('-',first);}
    [Fact] public void creation_token_makes_repeated_request_idempotent(){var token=Guid.NewGuid();Assert.Equal(LabelTemplateKeyPolicy.Generate(token),LabelTemplateKeyPolicy.Generate(token));}
    [Theory] [InlineData("ABC",true)] [InlineData("ABC_123",true)] [InlineData("A-B",false)] [InlineData("A B",false)] [InlineData("ÁBC",false)] public void key_contract_is_centralized(string key,bool expected)=>Assert.Equal(expected,LabelTemplateKeyPolicy.IsValid(key));
