...
public static class LabelsCanvasProductionRc25QualityCheck
{
    ...
    public static async Task<int> RunAsync(string root,string? connectionString,TextWriter output,TextWriter error,CancellationToken ct)
    {
        ...
        var publishedSnapshotReader=repository.Contains("snapshot_json::text SnapshotJson")&&repository.Contains("LabelCanvasVersionSnapshotReader.Read");
        ...
        Check(repository.Contains("GetPublishedAsync")&&repository.Contains("BeginRevisionAsync")&&(repository.Contains("join lateral")||publishedSnapshotReader)&&repository.Contains("BuildVersionSnapshot"),"Versão publicada e revisão","Snapshot/envelope ou separação draft/publicado ausente.");
