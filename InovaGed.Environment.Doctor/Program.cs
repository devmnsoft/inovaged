...
try
{
    ...
    var allowed = new[] { "quality-gate", "schema", "di", "routes", "razor", "icons", "dapper-mapping" };
    if (!allowed.Contains(command, StringComparer.OrdinalIgnoreCase)) { Console.Error.WriteLine("Uso: check [--json] | report | quality-gate|schema|di|routes|razor|icons|dapper-mapping [--no-db-required]"); return 3; }
    ...
    var selected=command switch{"schema"=>all.Where(x=>x is PostgresSchemaQualityCheck),"di"=>all.Where(x=>x is DependencyInjectionQualityCheck),"routes"=>all.Where(x=>x is RouteSmokeQualityCheck),"razor"=>all.Where(x=>x is RazorQualityCheck),"icons"=>all.Where(x=>x is IconQualityCheck),"dapper-mapping"=>all.Where(x=>x is DapperMappingQualityCheck),_=>all.AsEnumerable()};
