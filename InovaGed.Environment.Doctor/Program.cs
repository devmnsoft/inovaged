...
try
{
    ...
    var allowed = new[] { "quality-gate", "schema", "di", "routes", "razor", "icons" };
    if (!allowed.Contains(command, StringComparer.OrdinalIgnoreCase)) { Console.Error.WriteLine("Uso: check [--json] | report | quality-gate|schema|di|routes|razor|icons [--no-db-required]"); return 3; }
    ...
    IQualityCheck[] all=[new MigrationFileQualityCheck(),new DependencyInjectionQualityCheck(),new PostgresSchemaQualityCheck(),new RazorQualityCheck(),new IconQualityCheck(),new RouteSmokeQualityCheck(),new AdministrationLinksQualityCheck(),new LayoutQualityCheck()];
    var selected=command switch{"schema"=>all.Where(x=>x is PostgresSchemaQualityCheck),"di"=>all.Where(x=>x is DependencyInjectionQualityCheck),"routes"=>all.Where(x=>x is RouteSmokeQualityCheck),"razor"=>all.Where(x=>x is RazorQualityCheck),"icons"=>all.Where(x=>x is IconQualityCheck),_=>all.AsEnumerable()};
