...
try
{
    ...
    var allowed = new[] { "quality-gate", "release-readiness", "schema", "schema-check", "di", "di-check", "routes", "route-smoke", "razor", "razor-check", "icons", "icon-check", "dapper-mapping", "admin-links-check", "layout-check", "incidents", "incident-center-check" };
    if (!allowed.Contains(command, StringComparer.OrdinalIgnoreCase)) { Console.Error.WriteLine("Uso: check [--json] | report | quality-gate|release-readiness|route-smoke|schema-check|di-check|razor-check|icon-check|dapper-mapping|admin-links-check|incident-center-check|database-readiness [--no-db-required]"); return 3; }
    ...
    await QualityReportWriter.WriteAsync(root,report,CancellationToken.None);if(command.Equals("release-readiness",StringComparison.OrdinalIgnoreCase))await ReleaseReadinessReportWriter.WriteAsync(root,report,CancellationToken.None);foreach(var x in report.Checks)Console.WriteLine($"[{x.Status.ToString().ToUpperInvariant()}] {x.Check}: {x.Message}");Console.WriteLine($"Status geral: {report.Status.ToString().ToUpperInvariant()} | Relatórios: {(command.Equals("release-readiness",StringComparison.OrdinalIgnoreCase)?"artifacts/release":"artifacts/quality-gate")}");return report.Status==QualityStatus.Fail?2:report.Status==QualityStatus.Warning?1:0;
}
