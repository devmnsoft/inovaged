...
public sealed class PortabilityController(IPortabilityExportService exports, IPortabilityManifestService manifests) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreatePortabilityExportRequest request, [FromHeader(Name="Idempotency-Key")] string? key, CancellationToken ct)
    {
        ...
        var result = await exports.RequestAsync(request.TenantId, request.Scope, User.Identity?.Name ?? "api", key, HttpContext.TraceIdentifier, ct);
        return AcceptedAtAction(nameof(Get), new { id = result.Id }, result);
    }
    [HttpGet("exports/{id:guid}")] public async Task<IActionResult> Get(Guid id, [FromQuery] Guid? tenantId, CancellationToken ct) => (await exports.GetAsync(id, tenantId, ct)) is { } e ? Ok(e) : NotFound();
    ...
    [HttpPost("exports/{id:guid}/cancel")] public async Task<IActionResult> Cancel(Guid id, [FromQuery] Guid? tenantId, CancellationToken ct) => await exports.CancelAsync(id, tenantId, User.Identity?.Name ?? "api", ct) ? NoContent() : NotFound();
